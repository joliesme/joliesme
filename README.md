# Hi, I'm Jolies 👋

I'm a B2B product marketer in Singapore who builds products on the side to stay honest about what the tools can actually do.

By day I run go-to-market strategy and campaign analytics for enterprise connectivity and data centre products across APAC. On weekends I ship small AI-powered apps built spec-first with Claude Code.

This account is where I publish the working parts of that overlap: the Claude skills I use for real marketing work, the attribution methodology I trust enough to put in front of leadership, and the build logs from my side projects.

## What's here

[**claude-skills-for-marketers**](../../claude-skills-for-marketers)
Installable Claude skills generalized from the ones I use daily: a brand context template so Claude stops asking you the same questions, a campaign influence reporting methodology, and a spec-first app build framework.

[**honest-b2b-attribution**](../../honest-b2b-attribution)
How I report campaign influence without overclaiming. Vocabulary, funnel definitions, fuzzy matching logic, and the framing that keeps sales and finance in the room.

## A few things about how I work

* I write the spec before the prompt. Most AI build failures I've had were spec failures wearing a technical costume.
* I'd rather report a smaller number that survives scrutiny than a bigger one that doesn't.
* Bilingual (English / Mandarin), which turns out to matter a lot when your product logic depends on the Chinese almanac.

## Elsewhere

* ✍️ [Medium](https://medium.com/@joliestan) for the longer build stories
* 💼 [LinkedIn](https://linkedin.com/in/joliestan)
* 🗓️ [Zeri 择日](https://zer-dates.vercel.app), free and runs fully in your browser

